...
import copy
from algo.Dads.skill_dynamic import SKillDiscriminator
...
class ReplayBuffer:
    ...
    def store(self, obs, skill, act, rew, next_obs, done):
...
class SAC:
    ...
    def _loss(self, data):
        ...
        loss_sd = self.sd.increase_log(obs, skill, obs2)

        return loss_pi, loss_q, loss_sd

    def update(self, data):
        """ SD update"""
        self.sd_optimizer.zero_grad()
        loss_sd = self._loss(data)[2]
        loss_sd.backward()
        ...
        return loss_q.detach(), loss_pi.detach(), loss_sd
